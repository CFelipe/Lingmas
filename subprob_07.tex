...
\section{Resoluções}
    \begin{enumerate}
        ...
        \item
        (A única coisa que encontrei sobre compilação separada/independente
        é que ela é utilizada em  C/C++, como o livro comenta, Fortran II.
        Basicamente, compilação separada é poder compilar o código em
        partes (desde que suas dependências estejam inclusas na sua própria
        compilação) e então juntar, como exemplo os arquivos .o em C/C++)

        (Resumindo: sim, porque não? É muito melhor de se trabalhar assim,
        e não afeta a dificuldade da linguagem)
